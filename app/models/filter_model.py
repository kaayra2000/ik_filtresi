...
class FilterItem:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "preceding_operator": (
                self.preceding_operator.name if self.preceding_operator else None
            ),
        }
...
class FilterItems:
    ...
    def add(
        ...
        if not self._items:
            ...
            self._items.append(
                FilterItem(component=component, preceding_operator=operator)
            )
...
class FilterGroup(FilterComponent):
    ...
    def to_display_string(self, indent: int = 0) -> str:
        ...
        prefix = "  " * indent
        format_child = lambda c: (
            f"({c.to_display_string(0)})"
            if isinstance(c, FilterGroup)
            else c.to_display_string(0)
        )
        ...
        if "items" in data:
            for item_data in data["items"]:
                ...
                if group._items.is_empty():
                    ...
                    group._items.add(
                        item.component, item.preceding_operator or LogicalOperator.AND
                    )
