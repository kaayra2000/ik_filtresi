...
class CSVHandler(FileIOStrategy):
    ...
    def read(self, file_path: Path, **kwargs) -> pd.DataFrame:
        ...
        return pd.read_csv(
            file_path, delimiter=delimiter, encoding=encoding, parse_dates=True
        )
...
class ExcelHandler(FileIOStrategy):
    ...
    def read(self, file_path: Path, **kwargs) -> pd.DataFrame:
        ...
        return pd.read_excel(
            file_path, sheet_name=sheet_name, engine=engine, parse_dates=True
        )
...
class ODSHandler(FileIOStrategy):
    ...
    def read(self, file_path: Path, **kwargs) -> pd.DataFrame:
        ...
        sheet_name = kwargs.get("sheet_name", 0)
        return pd.read_excel(
            file_path, sheet_name=sheet_name, engine="odf", parse_dates=True
        )
    ...
    def write(self, df: pd.DataFrame, file_path: Path, **kwargs) -> bool:
        ...
        try:
            ...
            date_columns = df_copy.select_dtypes(
            ...
            for col in date_columns:
                ...
                df_copy.loc[non_null_mask, col] = df_copy.loc[
                    non_null_mask, col
                ].dt.strftime("%Y-%m-%d")
...
class JSONHandler(FileIOStrategy):
    ...
    def read(self, file_path: Path, **kwargs) -> pd.DataFrame:
        ...
        return pd.read_json(
            file_path, orient=orient, encoding=encoding, convert_dates=True
        )
...
class ParquetHandler(FileIOStrategy):
    ...
    def write(self, df: pd.DataFrame, file_path: Path, **kwargs) -> bool:
        ...
        try:
            ...
            df_copy.to_parquet(
                file_path, engine=engine, compression=compression, index=False
            )
            return True
        except Exception as e:
