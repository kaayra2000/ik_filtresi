...
class FilterEngine:
    ...
    def apply_filter_component(
        self, df: pd.DataFrame, component: FilterComponent
    ) -> pd.DataFrame:
        """
    ...
    def _apply_greater_than_or_equal(
        self, df: pd.DataFrame, f: FilterModel
    ) -> pd.Series:
        return df[f.column_name] >= f.value
    ...
    def _apply_contains(self, df: pd.DataFrame, f: FilterModel) -> pd.Series:
        return (
            df[f.column_name]
            .astype(str)
            .str.contains(str(f.value), case=False, na=False, regex=False)
        )
    ...
    def _apply_starts_with(self, df: pd.DataFrame, f: FilterModel) -> pd.Series:
        return (
            ...
            .astype(str)
            .str.lower()
            .str.startswith(str(f.value).lower(), na=False)
        )
    ...
    def _apply_ends_with(self, df: pd.DataFrame, f: FilterModel) -> pd.Series:
        return (
            ...
            .astype(str)
            .str.lower()
            .str.endswith(str(f.value).lower(), na=False)
        )
    ...
    def _apply_matches(self, df: pd.DataFrame, f: FilterModel) -> pd.Series:
        ...
        try:
            return (
                df[f.column_name]
                .astype(str)
                .str.match(str(f.value), case=False, na=False)
            )
        except re.error:
